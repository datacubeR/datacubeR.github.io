...
VS Code es bastante más sencillo. Si es que ya tenemos todo lo anterior configurado, sólo es necesario bajar la extensión `Remote SSH`, esto permite abrir una nueva sesión de VS Code desde [J.A.R.V.I.S]({{ site.baseurl }}/jarvis/). Para ello presiono `Ctrl+Shift+P` y busco `Remote-SSH: Connect to Host...`. Ingresando el comando de la forma `ssh usuario@host` podremos establecer sesión en VS Code.

![PENDIENTE]({{ site.urlimg }}){: .center}
