...
{% include alert success='Como se puede ver los cambios hechos al código son mínimos, y sólo se ingresaron los parámetros. De hecho, partes como evaluate_model.py casi no tiene código adicional. En estricto rigor, no agregamos nada que tenga que ver con el trackeo del proceso de experimentación.'%}
...
`dvc run` es el comando que permite crear una dependencia y a la vez ejecutar el DAG. Normalmente prefiero acompañarlo del flag `--no-exec` para realizar la ejecución después. El Script hace lo siguiente:
...
* `-p` son los parámetros, en caso de que cambien DVC volverá a ejecutar el DAG.
...
* Esta etapa se pidiendole a DVC que ejecute el comando `python src/03-train_model.py`.
...
Al hacer esto entonces DVC ha sido configurado y es una buena hacer un commit:

```shell
$ git add .
$ git commit -m 'Configurando DVC'
```
DVC incluso permite ver el Pipeline como un DAG, el cual podemos visualizar hacienddo lo siguiente:
...
DVC reconoce las distintas etapas y sus dependencias. Por lo que no nos queda más que ejecutar el proceso:
...
DVC entiende que ninguna de las dependencias ha cambiado. Por lo tanto, no ejecuta el proceso de nuevo, entendiendo que obtendríamos el mismo resultado.
...
¿Qué sucedería si quiero experimentar? Bueno para eso DVC posee comandos dedicados a dicha tarea. Probemos como le iría al modelo si es que C es 10 y 0.1
...
DVC entiende que modificar ese parámetro sólo tiene impacto en las etapas `train` y `evaluate`, por lo tanto no vuelve a ejecutar las primeras etapas (esto lo deduce de las dependencias dadas, por lo que hay que poner harto énfasis en configurar el DAG correctamente).
...
`dvc exp apply` se va a encargar de modificar nuestros archivos de tal modo que `dvc repro` nos entregue el mismo resultado obtenido en la experimentación. Por lo que puede modificar código, parámetros o data con tal de hacer el experimento reproducible.
...
¿No sería espectacular poder tener feedback de los procesos de entrenamiento de manera inmediata y que pudieramos tener el proceso de entrenamiento super automatizado usando CI/CD? 

Bueno, se viene la semana que sigue...
